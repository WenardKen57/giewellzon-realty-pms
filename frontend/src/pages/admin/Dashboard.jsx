...
export default function DashboardPreview({ previewCount = 6 }) {
  ...
  if (!metrics) {
    return (
      <div className="p-6 text-center text-red-500">No dashboard data available.</div>
    );
  }
  ...
  const occupancyPercent = totalUnits ? Math.round((soldUnits / totalUnits) * 100) : 0;
    ...
    <div className="p-6 space-y-8">
            ...
            extra={
              <div className="mt-3">
                ...
                <div className="mt-2 text-xs text-neutral-500">
                  Occupancy: <span className="font-medium text-neutral-700">{occupancyPercent}%</span>
                </div>
              </div>
            }
          />
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title="Available Units" value={availableUnits} variant="units" />
          <StatCard title="Sold Units" value={soldUnits} variant="sold" />
          <StatCard title="Rented Units" value={rentedUnits} variant="rented" />
          <StatCard title="Closed Sales" value={totalClosedSales} variant="sales" />
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2">
            ...
            <MiniStat label="Commission" value={formatPeso(totalCommissionPaid)} />
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <h3 className="text-md font-semibold text-gray-700">Recent Inquiries</h3>
              <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">
                {recentInquiries.length}
              </span>
            </div>
            <Link to="/admin/inquiries" className="text-sm text-brand-primary hover:underline">
              See all
            </Link>
          </div>
      ...
      <section className="bg-white p-5 rounded-2xl shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <h3 className="text-md font-semibold text-gray-700">Recent Sales</h3>
            <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">
              {recentSales.length}
            </span>
          </div>
          <Link to="/admin/sales" className="text-sm text-brand-primary hover:underline">
            See all
          </Link>
        </div>
...
function InfoTile({ title, value, description, variant = "default", icon, extra }) {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-50 flex items-start gap-4">
      <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${gradient} flex-shrink-0`}>
        {tileIcon(icon)}
      </div>
...
function tileIcon(name) {
  const baseProps = { width: 20, height: 20, fill: "none", className: "text-white" };
  if (name === "buildings") {
    return (
      <svg {...baseProps} viewBox="0 0 24 24" aria-hidden>
        <path d="M3 21V7a1 1 0 0 1 1-1h12v16H3z" fill="rgba(255,255,255,0.92)"></path>
  ...
  if (name === "units") {
    return (
      <svg {...baseProps} viewBox="0 0 24 24" aria-hidden>
        <rect x="3" y="4" width="18" height="6" rx="1.5" fill="rgba(255,255,255,0.9)"></rect>
        <rect x="3" y="14" width="7" height="6" rx="1.5" fill="rgba(255,255,255,0.9)"></rect>
      </svg>
    );
  }
  if (name === "inquiries") {
    return (
      <svg {...baseProps} viewBox="0 0 24 24" aria-hidden>
        <path d="M21 6v9a2 2 0 0 1-2 2H8l-5 4V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" fill="rgba(255,255,255,0.92)"></path>
      </svg>
    );
  }
  // fallback
  ...
  if (variant === "units") {
    return (
      <svg {...base} viewBox="0 0 24 24" aria-hidden>
        <rect x="3" y="3" width="18" height="7" rx="1.5" fill="white" opacity="0.9" />
        <rect x="3" y="14" width="7" height="7" rx="1.5" fill="white" opacity="0.9" />
      </svg>
    );
  }
  if (variant === "sold") {
    return (
      <svg {...base} viewBox="0 0 24 24" aria-hidden>
        <path d="M5 12h14" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
  ...
  if (variant === "rented") {
    return (
      <svg {...base} viewBox="0 0 24 24" aria-hidden>
        ...
        <rect x="5" y="13" width="14" height="6" rx="1" fill="white" opacity="0.95" />
      </svg>
    );
  }
  if (variant === "sales") {
  ...
  return (
    <div className="divide-y divide-gray-100">
      {items.slice(0, previewCount).map((it, idx) => {
        ...
        const title = it.subject || it.propertyName || it.buildingName || it.customerName || it.clientName || (type === "sale" && (it.reference || `Sale #${id}`)) || `#${id}`;
        const subtitle = it.message?.slice(0, 80) || it.unitName || it.propertyAddress || (it.amount ? `Amount: ${formatPeso(it.amount)}` : "");
        ...
        const to = type === "inquiry" ? `/admin/inquiries/${id}` : type === "sale" ? `/admin/sales/${id}` : "#";

        return (
          <Link
            key={String(id || idx)}
            to={to}
            className={`flex items-start gap-3 p-3 hover:bg-green-50 transition ${compact ? "py-2" : ""}`}
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {showAgent && agent && <span className="text-xs text-neutral-500 truncate max-w-[90px]">{agent}</span>}
                  ...
                  <p className="text-xs text-neutral-500">{formatShortDate(date)}</p>
                </div>
              </div>
            </div>
          </Link>
...
function statusBadgeClass(status) {
  ...
  if (s.includes("handled") || s.includes("closed") || s.includes("done") || s.includes("completed")) return "bg-green-100 text-green-800";
  if (s.includes("rejected") || s.includes("cancel")) return "bg-red-100 text-red-800";
